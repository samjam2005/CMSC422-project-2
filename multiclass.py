    ...
    def predict(self, X, useZeroOne=False):
        ...
        for i in range(self.K):
            for j in range(i):
                ...
                p_j = probs[0, 1]      # probability of class j (+1)
                p_i = probs[0, 0]      # probability of class i (-1)
...
class MCTree:
    ...
    def train(self, X, Y):
        for n in self.tree.iterNodes():
            ...
            mask = array([(y in leftLabels or y in rightLabels) for y in Y])
            thisX = X[mask, :]
            thisY_raw = Y[mask]

            leftSet = set(leftLabels)
            thisY = array([1 if y in leftSet else -1 for y in thisY_raw])
    ...
    def predict(self, X):
        ...
        while not node.isLeaf:
            clf = node.getNodeInfo()
            pred = clf.predict(X_reshaped)[0]
            if pred > 0:
                node = node.getLeft()
            else:
                node = node.getRight()
